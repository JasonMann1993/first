...
use App\Api\Requests\DeliverWaterRequest;
use App\Models\Img;
...
class DeliverWaterController extends BaseController
{
    ...
    public function index(DeliverWaterRequest $request)
    {
        ...
        $list = app(DeliverWater::class)->getNearLists($longitude, $latitude, $where);
        $list = new LengthAwarePaginator($list->forPage($pageNow, $pageSize), $list->count(), $pageSize);
        return $this->response->paginator($list, new DeliverWaterTransformer);
    }

    public function publish(DeliverWaterRequest $request)
    {
        $userInfo = User::where('openid', $request->get('id'))->first();
        $ins = new DeliverWater();
        ...
        $tmpIns->common_type = DeliverWater::class;
        try {
            $tmpIns->save();
        } catch (\Exception $error) {
            \DB::rollBack();
